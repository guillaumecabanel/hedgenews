class ArticlesController < ApplicationController
  ...
  def index
    ...
    if Topic.pluck(:name).include?(@topic_search) # topic already exists
      if Time.now - Topic.find_by_name(@topic_search).articles.last.created_at < 86400 # last article was added to the topic less than 24 hours
      ...
      else # last article was added more than 24 hours ago. launch research again
        @topic = Topic.find_by_name(@topic_search)
        ...
        @stories.each do |story|
          ...
          unless @topic.articles.pluck(:aylien_id).include?(article.aylien_id) || @topic.articles.pluck(:title).include?(article.title) # possible that two articles have the same title but not the same source. Change code ? add a condition
